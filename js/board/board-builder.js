/* ═══════════════════════════════════════════════════════════════
   BOARD BUILDER — generates the SVG board from SQ data
   تعديل هذا الملف يغير شكل خلايا اللوحة فقط
   يعتمد على: js/data/board-squares.js (SQ, sqMap, PC)
═══════════════════════════════════════════════════════════════ */
'use strict';

const B   = 660;     // board total size px
const C   = 96;      // corner cell size px
const S   = (B - 2 * C) / 9;   // side cell width px
const CB  = 18;      // color band thickness px

// Background colors for special cells
const specBg = {
...
/**
 * Generate SVG markup for one board cell
 */
...
${s.s2 ? `<text x="${cx}" y="${cy+22}" text-anchor="middle" font-size="6.5" fill="rgba(255,215,0,.45)">${s.s2}</text>` : ''}
  ...
  const nb = s.ml === 'navy';
  const bg = s.t === 'prop'
    ? (nb ? 'url(#navyCell)' : 'rgba(255,250,232,.97)')
    : (specBg[s.t] || 'rgba(245,232,200,.92)');
  ...
  let pt = '';
  if(s.p)
    pt = `<text x="${cx}" y="${cy+19}" text-anchor="middle" font-size="5.8" fill="#3a2000" font-family="'Courier New',monospace" dominant-baseline="middle">${s.p} din</text>`;
  else if(s.sub)
    pt = `<text x="${cx}" y="${cy+19}" text-anchor="middle" font-size="6.5" fill="#8A0000" font-weight="bold" font-family="monospace" dominant-baseline="middle">-${s.sub} din</text>`;
  else if(s.t === 'gate')
    pt = `<text x="${cx}" y="${cy+19}" text-anchor="middle" font-size="5.8" fill="#10105A" font-family="monospace" dominant-baseline="middle">200 din</text>`;
  else if(s.t === 'util')
    pt = `<text x="${cx}" y="${cy+19}" text-anchor="middle" font-size="5.8" fill="#0A3A3A" font-family="monospace" dominant-baseline="middle">150 din</text>`;
...
/** Build and inject the full board SVG */
function buildBoard(){
  const mid = B / 2;
  let cells = '';
  ...
  for(let i = 0; i <= 10; i++){
    const ic = i === 0 || i === 10, w = ic ? C : S;
    const x = i === 0 ? B - C : i === 10 ? 0 : B - C - i * S;
    cells += buildCell(i, x, B - C, w, C, 'bottom');
  }
  ...
  for(let i = 0; i <= 10; i++){
    ...
    cells += buildCell(20 + i, x, 0, w, C, 'top');
  }
...
<radialGradient id="centerBg" cx="50%" cy="50%" r="55%">
  <stop offset="0%" stop-color="#2C1400" stop-opacity=".97"/><stop offset="100%" stop-color="#100600" stop-opacity=".99"/>
</radialGradient>
<linearGradient id="navyBand" x1="0%" y1="0%" x2="100%" y2="0%">
  <stop offset="0%" stop-color="#040C28"/><stop offset="50%" stop-color="#3858D8"/><stop offset="100%" stop-color="#040C28"/>
</linearGradient>
<linearGradient id="navyCell" x1="0%" y1="0%" x2="100%" y2="100%">
  <stop offset="0%" stop-color="#F0F4FF"/><stop offset="100%" stop-color="#D5DEFF"/>
</linearGradient>
...
// Build board immediately on load
buildBoard();
